...
def recover_data():
    ...
    tonnage_total_annee_verre = tonnage_total_annee_type\
        .filter(ANNEE=2019, C_TYP_REG_DECHET='08C')\
        .first().get('tonnage_total')

    tonnage_total_annee_plastique = tonnage_total_annee_type\
        .filter(ANNEE=2019, C_TYP_REG_DECHET='08B')\
        .first().get('tonnage_total')

    tonnage_total_annee_menager = tonnage_total_annee_type\
        .filter(ANNEE=2019, C_TYP_REG_DECHET='08A')\
        .first().get('tonnage_total')

    tonnage_total_verre = [item.get('tonnage_total') for item in
                           sorted(tonnage_total_annee_type.filter(C_TYP_REG_DECHET='08C'), key=lambda x: x.get('ANNEE'))]

    tonnage_total_plastique = [item.get('tonnage_total') for item in
                               sorted(tonnage_total_annee_type.filter(C_TYP_REG_DECHET='08B'), key=lambda x: x.get('ANNEE'))]

    tonnage_total_menager = [item.get('tonnage_total') for item in
                             sorted(tonnage_total_annee_type.filter(C_TYP_REG_DECHET='08A'), key=lambda x: x.get('ANNEE'))]

    return int(tonnage_total_france), int(tonnage_total_annee_verre),int(tonnage_total_annee_plastique),int(tonnage_total_annee_menager), tonnage_total_region_annee_type,int(diff_tonnage_france_total), tonnage_total_verre,tonnage_total_plastique, tonnage_total_menager
